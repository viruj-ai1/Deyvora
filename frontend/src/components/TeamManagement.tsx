...
import { UserPlus, Users, UserSquare2, KeyRound, Eye, EyeOff, CheckCircle, X, Copy, BadgeCheck, ChevronDown, UserCheck } from 'lucide-react';
...
export const TeamManagement = () => {
  ...
  return (
    <div className="space-y-6 fade-in">
      {/* Header with Top Right Provision Member Dropdown */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        {/* Provision Member Dropdown Button */}
        <div className="relative">
          <button
            type="button"
            onClick={() => setIsDropdownOpen(p => !p)}
            className="bg-[#1e3a5f] hover:bg-[#162d4a] text-white px-5 py-2.5 rounded-xl font-bold text-sm flex items-center gap-2 shadow-sm hover:shadow-md transition-all active:scale-[0.98]"
          >
          ...
          {isDropdownOpen && (
            <>
              ...
              />
              <div className="absolute right-0 mt-2 w-60 bg-white rounded-2xl shadow-xl border border-gray-100 py-2 z-20 fade-in">
                <button
                  type="button"
                  onClick={() => {
                ...
                >
                  ...
                  <div>
                    <div className="font-bold text-gray-900">Create New</div>
                    <div className="text-[11px] text-gray-400 font-normal">Register a new user account</div>
                  </div>
                </button>
                ...
                >
                  ...
                  <div>
                    <div className="font-bold text-gray-900">Add Existing</div>
                    <div className="text-[11px] text-gray-400 font-normal">Add existing user to team</div>
                  </div>
                </button>
              </div>
            </>
      ...
      {/* ── Direct Reports Panel (Full Width) ── */}
        ...
        <div className="p-6">
          {directReports.length === 0 ? (
            <div className="p-12 text-center text-gray-400 border-2 border-dashed border-gray-200 rounded-2xl bg-gray-50/50">
              ...
              <p className="text-sm mt-1">Click <strong>Provision Member</strong> at the top right to add team members.</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {directReports.map((user: any) => {
                ...
                return (
                  <div key={user.id} className="bg-white border border-gray-200 rounded-2xl overflow-hidden hover:border-blue-300 hover:shadow-sm transition-all flex flex-col justify-between">
                    {/* User info row */}
                    <div className="group flex items-start justify-between p-4 gap-3">
                      <div className="flex items-center gap-3.5">
                        <div className={`p-3 rounded-xl transition-colors shrink-0 ${isSelf ? 'bg-amber-50 text-amber-600' : 'bg-gray-50 text-gray-400 group-hover:bg-blue-50 group-hover:text-blue-500'}`}>
                      ...
                      {!isSelf && (
                        <button
                          ...
                          className="flex items-center gap-1.5 text-[12px] font-bold px-3 py-1.5 rounded-lg transition-all shrink-0"
                          style={{
                      ...
                      <div className="border-t border-gray-100 bg-blue-50/50 px-4 py-4 fade-in">
                        {resetStatus === 'success' ? (
                          <div className="flex items-center gap-2 text-green-700 font-semibold text-sm">
                            <CheckCircle className="w-4 h-4 shrink-0" /> Password reset successfully! Share the new credentials with {user.name}.
                          </div>
                        ) : (
                          <>
                            <p className="text-[12px] font-semibold text-gray-500 mb-2">Set a new password for <span className="text-gray-800">{user.name}</span></p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-900/60 backdrop-blur-sm fade-in">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden border border-gray-100">
            ...
            {provisionMode === 'new' ? (
              <form onSubmit={handleAddUser} className="p-6 space-y-4 max-h-[75vh] overflow-y-auto">
                <div>
                  ...
                  <input
                    required
                    type="text"
                    className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl outline-none focus:bg-white focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400 transition-all text-sm font-medium text-gray-900"
                    value={name}
                    onChange={e => setName(e.target.value)}
                    placeholder="e.g. John Doe (Analyst)"
                  />
                </div>
                <div>
                  <label className="block text-[12px] font-bold text-gray-600 mb-1.5 uppercase tracking-wide">
                  ...
                  {loginIdError ? (
                    ...
                    <p className="text-[11px] text-gray-400 mt-1.5">
                      This will be the member's login username.
                    </p>
                  )}
                </div>
                <div>
                ...
                <div>
                  ...
                  <p className="text-[11px] text-amber-600 mt-1.5 font-medium">
                    ⚠ Hand these credentials to the user directly. They can change their password after first login.
                  </p>
                </div>
                ...
                {newCredential && (
                  <div className="relative rounded-2xl border border-emerald-200 bg-emerald-50 p-4 fade-in">
                    ...
                    <p className="text-[11px] text-emerald-600 mb-3 font-medium">
                      Share these details with <span className="font-bold">{newCredential.name}</span>.
                    </p>
                    ...
                    <div className="mb-2">
                      ...
                      <div className="flex items-center gap-2 bg-white border border-emerald-200 rounded-xl px-3 py-2">
                        <code className="flex-1 text-[13px] font-mono font-bold text-gray-800 break-all">
                          {newCredential.id}
                        </code>
                        <button
                          type="button"
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2 bg-white border border-emerald-200 rounded-xl px-3 py-2">
                        <code className="flex-1 text-[13px] font-mono font-bold text-gray-800">
                ...
                <div className="pt-2 flex items-center justify-end gap-3 border-t border-gray-100">
                  <button
                    ...
                    className="px-5 py-2.5 rounded-xl border border-gray-200 text-gray-600 font-bold hover:bg-gray-50 transition-all text-sm"
                  >
                    Close
                  </button>
                  <button
                    type="submit"
                    className="bg-[#1e3a5f] text-white px-6 py-2.5 rounded-xl font-bold hover:bg-[#162d4a] shadow-sm transition-all active:scale-[0.98] text-sm"
                  >
                    Add to Hierarchy
                  </button>
                </div>
              ...
              <form onSubmit={handleAddExistingUser} className="p-6 space-y-4">
                <div>
                  <label className="block text-[12px] font-bold text-gray-600 mb-1.5 uppercase tracking-wide">
                  ...
                  >
                    ...
                    {availableExistingUsers.map((u: any) => (
